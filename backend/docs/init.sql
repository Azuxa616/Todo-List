...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Todolist 任务表' ROW_FORMAT = Dynamic;

ALTER TABLE `task` ADD INDEX `task_user_id_index`(`user_id`);
ALTER TABLE `task` ADD INDEX `task_status_index`(`status`);
ALTER TABLE `task` ADD INDEX `task_star_index`(`star`);
ALTER TABLE `task` ADD INDEX `task_due_date_index`(`due_date`);
ALTER TABLE `task` ADD INDEX `task_create_time_index`(`create_time`);
ALTER TABLE `task` ADD INDEX `task_update_time_index`(`update_time`);
...
ALTER TABLE `category` ADD INDEX `category_user_id_index`(`user_id`);
...
CREATE TABLE `tags` (
                        ...
                        FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '任务标签表' ROW_FORMAT = Dynamic;

-- 索引优化
ALTER TABLE `tags` ADD INDEX `tags_user_id_index`(`user_id`);
ALTER TABLE `tags` ADD INDEX `tags_name_index`(`name`);
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '任务与标签的关联表' ROW_FORMAT = Dynamic;

ALTER TABLE `task` ADD COLUMN `category_id` BIGINT NULL COMMENT '任务所属分类ID';
ALTER TABLE `task` ADD CONSTRAINT `fk_task_category`
    FOREIGN KEY (`category_id`) REFERENCES `category` (`id`) ON DELETE SET NULL;
ALTER TABLE `task` ADD INDEX `task_category_id_index`(`category_id`);